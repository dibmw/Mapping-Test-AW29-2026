...
export default function App() {
  ...
  const [selectedSiswa, setSelectedSiswa] = useState<SiswaSchedule | null>(null);
  ...
  useEffect(() => {
    async function fetchData() {
      const csvUrl = "https://docs.google.com/spreadsheets/d/1m7n1Lf6oEO7xyRayp6J9aleY-3E60upiqOshM0mNKsw/export?format=csv&gid=1757285573";
      try {
        setIsLoading(true);
        setError(null);
        ...
        const uniqueRuang = new Set(parsedData.map(s => s.ruang).filter(Boolean));
        const uniqueHari = new Set(parsedData.map(s => s.day).filter(Boolean));
        setStats({
        ...
        setError(err.message || "Koneksi internet bermasalah atau spreadsheet tidak dapat diakses.");
      } finally {
        setIsLoading(false);
      }
    }
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-6 md:py-10 space-y-8 print:p-0 print:max-w-full">
        ...
        <section className="text-center space-y-3.5 print:hidden">
          <h2 className="font-display font-extrabold text-2xl md:text-3xl text-gray-900 tracking-tight leading-tight">
            Cek Jadwal <span className="text-brand-green underline decoration-brand-gold decoration-4 underline-offset-4">Mapping Test</span> Siswa
          </h2>
          <p className="text-xs md:text-sm text-gray-500 max-w-md mx-auto leading-relaxed">
            Gunakan kolom di bawah untuk mencari jadwal ujian ananda dengan mudah dan cepat.
          </p>
        </section>
          ...
          <AnimatePresence mode="wait">
            {isLoading && (
              ...
              >
                <div className="relative">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center text-[10px] text-brand-green font-bold">AL</div>
                </div>
                <div className="space-y-1">
                  <h4 className="font-display font-bold text-gray-800 text-sm md:text-base">Menghubungkan ke Google Sheets...</h4>
                  <p className="text-xs text-gray-400">Sinkronisasi database jadwal real-time sedang berlangsung</p>
                </div>
              </motion.div>
            )}
            ...
            {error && (
              ...
              >
                ...
                <div className="space-y-1">
                  <h4 className="font-display font-bold text-red-800 text-sm md:text-base">Gagal Sinkronisasi</h4>
                  <p className="text-xs text-red-600 max-w-sm mx-auto leading-relaxed">{error}</p>
                </div>
                <button
                  type="button"
                ...
                <div className="bg-white p-5 md:p-6 rounded-2xl shadow-md border border-gray-100 space-y-4">
                  ...
                  {!selectedSiswa && (
                    <div className="pt-2 text-xs text-gray-400 flex flex-wrap items-center gap-1.5">
                      <span className="font-medium text-gray-500">Contoh pencarian:</span>
                      <span className="font-semibold text-gray-600 bg-gray-100 px-2.5 py-0.5 rounded-lg">Abdullah</span>
                      <span className="text-gray-400">atau</span>
                      <span className="font-semibold text-gray-600 bg-gray-100 px-2.5 py-0.5 rounded-lg">Jasmine</span>
                    </div>
                  )}
                </div>
        ...
        <section className="bg-brand-gold-soft border border-brand-gold/10 p-5 rounded-2xl flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between print:hidden">
          <div className="space-y-1">
            ...
            <p className="text-xs text-gray-500 max-w-sm leading-relaxed">
              Jika Abu/Ummu membutuhkan informasi lebih lanjut, Insya Allah dapat menghubungi wali kelas ananda.
            </p>
          </div>
        </section>
      ...
      <footer className="bg-brand-green-dark text-white py-6 mt-auto print:hidden">
        <div className="max-w-2xl mx-auto px-4 flex flex-col sm:flex-row justify-between items-center text-[11px] text-brand-green-soft/60 gap-3 font-sans">
          ...
          <div className="flex gap-4">
            <a href="#ketentuan-pelaksanaan" className="hover:text-brand-gold text-brand-green-soft/80 transition-colors font-medium">
